...
class ProductPopup {
  ...
  load(product, variation, row, type) {
    ...
    let retailBodiShop = window.inventoryListModel.cal.calculateRetailUniversal(
      product,
      variation,
      "bodishop",
    );
    ...
    this.state.type = ["stock", "avg", "order"].includes(type)
      ? type
      : this.state.type;
    ...
    this.$el
      ...
      .attr(
        "src",
        `./../images/${slugify(this.product.category)}/${this.product.getImageName()}_1.jpg`,
      );
  ...
  _onOverLayClick(e) {
    ...
    if (!$popup.has($(e.target)).length && !$popup.is($(e.target))) {
      this.hide();
    }
  }
  _onCellSelect(e) {
  ...
  _addToCart() {
    ...
    const orderPrice = this.variation.Markup ?? 0;
    const orderCost = window.inventoryListModel.cal.calculateCost(
      product,
      this.variation,
    );
    let ReqData = {
  ...
  _updateStock() {
    ...
    stockRetailShop = isNaN(this.state.stock)
      ? 0
      : parseFloat(this.state.stock);
    RetailSize = variation.RetailSize;
    RetailUnit = variation.RetailUnit;
    convertedToGr =
      this.convert.convertStockValue(RetailSize, RetailUnit, "gr") *
      stockRetailShop;
  ...
  _updateDisplay() {
    this.$el
      .find("#product-name")
      .text(
        `${this.product.Product} - ${this.variation.RetailSize} ${this.variation.RetailUnit}`,
      );
    this.$el.find(".stock").text(Math.round(this.state.stock));
    this.$el
      .find(".price")
      .text((Math.round(this.state.price * 100) / 100).toFixed(2));
    ...
    this.$el
      .find(`.cell-select[data-key="${this.state.cellSelected}"]`)
      .addClass("active");
  }
