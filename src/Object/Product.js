export class Product {
  ...
  getImageName() {
    return this.Product.replace(/ /g, "_").replace(
      /[:\/'"?@#$%&*()\-+|<>]/g,
      "",
    );
  }
  slugify() {
    return this.Product.replace(/ /g, "_");
  }

  getThumbnailId() {
    if (Array.isArray(this.Images) && this.Images.length) {
      let images = this.Images.sort(
        (a, b) =>
          parseInt(isNaN(a.sequence) ? 0 : a.sequence) -
          parseInt(isNaN(b.sequence) ? 0 : b.sequence),
      );
